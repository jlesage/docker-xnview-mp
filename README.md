...
> [!NOTE]
> This Docker container is entirely unofficial and not made by the creators of XnView MP.
...
server {
	...
	location /xnview-mp/ {
		...
		location ~ ^/xnview-mp/(websockify(-.*)?) {
                        proxy_pass http://docker-xnview-mp/$1;
			proxy_http_version 1.1;
			proxy_set_header Upgrade $http_upgrade;
			proxy_set_header Connection $connection_upgrade;
			proxy_read_timeout 86400;
...
| **Application version** label | Displays the version of XnView MP integrated into Docker image. |
...
Any desktop notification generated by XnView MP is forwarded to the
